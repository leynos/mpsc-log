...
| Object path                  | A `jo`-style field path that writes a value into a nested JSON object rather than into a literal top-level key.                                                        |
...
| Type coercion                | Conversion of a CLI value to a JSON string, number, boolean, null, object, or array according to explicit flags, sidecar schema, or default `jo`-compatible inference. |
